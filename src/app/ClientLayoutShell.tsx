...
export default function ClientLayoutShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      gradient.addColorStop(0, `rgba(${40 + 50 * Math.sin(gradientShift)},20,100,1)`);
      gradient.addColorStop(0.5, `rgba(80,${40 + 40 * Math.cos(gradientShift)},160,1)`);
      gradient.addColorStop(1, `rgba(255,${140 + 20 * Math.sin(gradientShift)},0,1)`);
      ...
      shapes.forEach((shape) => {
        ...
        ctx.shadowBlur =
          shape.color.r === 255 && shape.color.g === 140 && shape.color.b === 0 ? 25 : 15;
    ...
    <>
      ...
      <header
        className={`z-50 border-b ${
          isClient ? "sticky top-0 backdrop-blur-md bg-white/10" : "relative bg-transparent"
        }`}
      >
