...
export default function GameCodeQrCard({ gameCode, onCloseHref = "/" }: Props) {
  ...
  return (
    <section className="rounded-2xl border border-white/15 bg-black/25 p-5">
      ...
      <h2 className="mt-2 font-mono text-3xl font-bold tracking-[0.1em]">{parsedCode.isValid ? parsedCode.value : "------"}</h2>
      ...
      <div className="mt-5 inline-flex rounded-xl border border-white/20 bg-white p-4">
        {joinLink ? (
          ...
          <div className="flex h-[170px] w-[170px] items-center justify-center rounded-lg border border-black/10 bg-white text-center text-xs text-black/70">
            Valid game code required
          </div>
        )}
      </div>
