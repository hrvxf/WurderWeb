...
import { BUSINESS_ROUTES, joinRoute, managerDashboardRoute } from "@/lib/business/routes";
...
type CompanyGameCreatedStateProps = {
...
export default function CompanyGameCreatedState({
  result,
  copyState,
  onCopyGameCode,
  onCopyJoinLink,
  onCreateAnother,
}: CompanyGameCreatedStateProps) {
  const hostDashboardHref = managerDashboardRoute(result.gameCode);
  ...
  return (
    <section className="animate-subtle-enter space-y-5 rounded-2xl border border-emerald-300/35 bg-emerald-500/10 p-5 sm:p-6">
      <div className="space-y-2">
        <p className="text-xs uppercase tracking-[0.2em] text-emerald-100/80">Session created</p>
        <h2 className="text-2xl font-semibold tracking-tight sm:text-3xl">Your session is ready</h2>
        <p className="text-sm text-emerald-100/85">
          Analytics are enabled for this session and will be available in manager reporting.
        </p>
        <p className="text-sm text-emerald-100/85">
          {isHostOnly
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <article className="rounded-xl border border-white/15 bg-black/20 px-4 py-3">
          <p className="text-xs uppercase tracking-[0.16em] text-white/70">Game code</p>
      ...
      <div className="rounded-xl border border-white/15 bg-black/20 p-4">
        <p className="text-xs uppercase tracking-[0.16em] text-white/70">Join QR code</p>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        >
          {copyState.gameCode ? "Game code copied" : "Copy game code"}
        </button>
        <button
          type="button"
          className="rounded-xl border border-white/25 bg-white/5 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-white/10"
